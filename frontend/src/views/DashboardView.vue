  ...
  <div class="pt-16 pb-20 md:pb-0 min-h-screen bg-surface">
    <div class="flex">
      ...
      <aside class="hidden md:block w-60 fixed top-16 left-0 bottom-0 bg-surface-light border-r border-surface-lighter overflow-y-auto p-4">
        <h3 class="text-xs font-semibold text-txt-dim uppercase tracking-wider mb-3">分类</h3>
        <div class="space-y-1">
          <button @click="selectCategory(null)"
            ...
            <Key class="w-4 h-4 inline mr-2" />全部密钥
          </button>
      ...
      <main class="flex-1 md:ml-60 p-4 lg:p-6">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h2 class="text-xl font-bold text-txt">
              {{ activeCategoryName || '全部密钥' }}
            </h2>
            <p class="text-txt-dim text-sm mt-1">共 {{ secrets.length }} 条记录</p>
          </div>
        ...
        <div v-else-if="secrets.length === 0" class="text-center py-20">
          ...
          <p class="text-txt-muted">暂无密钥记录</p>
          <router-link to="/secret/new" class="text-primary text-sm mt-2 inline-block hover:underline">添加第一个密钥</router-link>
        </div>
