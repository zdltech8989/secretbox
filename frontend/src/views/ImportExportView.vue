  ...
  <div class="pt-16 pb-20 md:pb-0 min-h-screen bg-surface">
    <div class="max-w-2xl mx-auto p-4 lg:p-6">
      ...
      <div class="glass-strong rounded-2xl p-6">
        <h3 class="text-lg font-semibold text-txt mb-4 flex items-center gap-2">
          <Download class="w-5 h-5 text-primary" />导入数据
        </h3>
        <p class="text-sm text-txt-dim mb-4">支持 Google Chrome / Microsoft Edge 密码导出的 CSV 文件</p>
        ...
        <div v-if="file" class="mt-3">
          <label class="block text-sm text-txt-muted mb-1">导入分类</label>
...
async function handleImport() {
  ...
  if (!auth.vaultUnlocked) {
    importResult.value = { error: '请先解锁保险箱' }
  ...
  try {
    ...
    importResult.value = { error: e.response?.data?.detail || '导入失败' }
